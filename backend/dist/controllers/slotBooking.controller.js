...
class SlotBookingController {
    constructor() {
      ...
      `; // Send emails asynchronously for faster response
                const sendEmails = async () => {
                    try {
                        ...
                        const [userEmailResult, adminEmailResult] = await Promise.allSettled([
                            mailer_1.transporter.sendMail(userEmailOptions),
                            mailer_1.transporter.sendMail(adminEmailOptions)
                        ]);
                        // Log results
                        if (userEmailResult.status === 'fulfilled') {
                            ...
                            logger_1.default.error(`Failed to send email to user ${user.email}:`, userEmailResult.reason);
                        }
                        if (adminEmailResult.status === 'fulfilled') {
                            ...
                            logger_1.default.error(`Failed to send email to admin about booking by ${user.email}:`, adminEmailResult.reason);
                        }
                        logger_1.default.info(`Slot booked successfully for user ${user.name || 'User'} (${user.email}) on ${date} at ${time}`);
                    }
                    catch (emailError) {
                        logger_1.default.error("Email sending error:", emailError);
        ...
        this.checkSlotAvailability = async (req, res) => {
            try {
