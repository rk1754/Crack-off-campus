    ...
    constructor() {
        this.uploadResume = async (req, res) => {
            try {
                ...
                const resume = await resume_model_1.default.create({
                    name,
                    description,
        ...
        this.getAllTemplates = async (req, res) => {
            try {
                const templates = await resume_model_1.default.findAll();
                if (!templates) {
                    res.status(404).json({
                        success: false,
        ...
        this.getAllTemplatesAdmin = async (req, res) => {
            try {
                const templates = await resume_model_1.default.findAll();
                if (!templates) {
                    res.status(404).json({
        ...
        this.downloadTemplate = async (req, res) => {
            try {
                const user = req.user;
                if (!user ||
                ...
                const template = await resume_model_1.default.findOne({
                    where: {
                        id: templateId,
                    },
                });
                ...
                if (template.type === "other_templates" &&
                    user.subscription_type !== "other_templates") {
                    res.status(403).json({
                        success: false,
                        message: "You are not allowed to download this template",
                ...
                const url = cloudinary_1.default.url(template.template_url, {
                    secure: true,
                    attachment: true,
                    resource_type: "raw",
                });
        ...
        this.downloadAllTemplates = async (req, res) => {
            try {
                const templates = await resume_model_1.default.findAll({
        ...
        this.downloadTemplateAdmin = async (req, res) => {
            try {
                const admin = req.admin;
                if (!admin) {
                    res.status(403).json({
        ...
        this.updateTemplateUpload = async (req, res) => {
            try {
                const templateId = req.params.id;
                if (!req.file) {
                    res.status(400).json({
                ...
                await cloudinary_1.default.api.delete_resources_by_prefix(`job-portal/resume_templates/${template.template_id}`, {
                    resource_type: "raw",
                });
                const base64 = `data:${file.mimetype};base64,${file.buffer.toString("base64")}`;
                const result = await cloudinary_1.default.uploader.upload(base64, {
        ...
        this.deleteTemplate = async (req, res) => {
            try {
                ...
                await Promise.all([
                    cloudinary_1.default.api.delete_resources_by_prefix(`job-portal/resume_templates/${template.template_id}`, {
                        resource_type: "raw",
                    }),
                    template.destroy(),
                ]);
        ...
        this.getTemplatesByType = async (req, res) => {
            try {
