...
const ALLOWED_ORIGINS = [FRONTEND_URL, FRONTEND_URL_2, FRONTEND_URL_3].filter(
  Boolean
) as string[];
  ...
  app.post("/update", async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const actualSubscriptionType =
        resourceToSubscriptionType[subscription_type];
      if (!actualSubscriptionType) {
        console.log("Invalid subscription type:", subscription_type);
        res.status(400).json({
          success: false,
          message: `Invalid subscription_type. Must be one of: ${Object.keys(
            resourceToSubscriptionType
          ).join(", ")}`,
        });
        return;
      }
      ...
      if (actualSubscriptionType === "basic") {
        ...
        actualSubscriptionType === "standard" ||
        actualSubscriptionType === "booster"
      ) {
        user.resume = true;
        user.referral = true;
        user.cold_mail = true;
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        for (const user of expiredUsers) {
          console.log(
            `Updating user ${user.id} - Subscription expired on ${user.subscription_expiry}`
          );
        ...
        console.log(
          `Successfully updated ${updatedCount} expired subscriptions`
        );
  ...
  setInterval(async () => {
    try {
      ...
      if (expiredUsers.length > 0) {
        console.log(
          `Found ${expiredUsers.length} users with expired subscriptions - updating to regular`
        );
        ...
        console.log(
          `Automatically updated ${updatedCount} expired subscriptions to regular`
        );
      } else {
